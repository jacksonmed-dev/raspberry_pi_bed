...
def body_part_location_update(bed: Bed, test_df, BODY_MODEL_DIR):
    # this step is taking forever when line 38 in main.py is not commented out
    from decision_algorithm.ml import preprocessing, model
    data = np.asarray(preprocessing.extract_sensor_dataframe(test_df['readings']),
...
def part1_adjustment(bed: Bed, test_df, BODY_MODEL_DIR):
    ...
    body_part_location_update(bed, test_df, BODY_MODEL_DIR)
    print(bed.get_tube_body_composition())
...
def algorithm_part2(bed: Bed, LSTM_MODEL_DIR, TEST_CSV_DIR):
    from decision_algorithm.ml import model
    ...
    # NEEDS TO BE ADAPTED FOR PASSING DATA-FRAME AND NOT READING IN CSV
...
def main_algorithm(bed: Bed):
    ...
    if len(ulcer) > 0:
        ...
        part1_adjustment(bed,test_df,BODY_MODEL_DIR)
    ...
    # for them accordingly
        ...
        algorithm_part2(bed, LSTM_MODEL_DIR, test_df)
